...
function useFacultyData() {
  ...
  const persist = (nextData) => {
    ...
    try {
      ...
      // demo mode: keep working in-memory even if persistence fails
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{ position: "sticky", top: 0, zIndex: 40, background: "var(--surface)", borderBottom: "1px solid var(--border)", width: "100%" }}>
      <div style={{ maxWidth: 1180, width: "100%", margin: "0 auto", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        <button onClick={() => goto("home")} style={{ display: "flex", alignItems: "center", gap: 12, background: "none", border: "none", flexShrink: 0, minWidth: 0 }}>
          {settings.logoImage ? (
            <img src={settings.logoImage} alt="logo" style={{ width: 52, height: 52, borderRadius: 14, objectFit: "cover", border: "1px solid var(--border)", flexShrink: 0 }} />
          ) : (
            <div style={{ width: 52, height: 52, borderRadius: 14, background: "linear-gradient(135deg, var(--primary-soft), var(--primary))", color: "var(--on-primary)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Noto Serif Thai',serif", fontWeight: 700, fontSize: 13, flexShrink: 0 }}>
              {settings.siteTitle}
            </div>
          )}
          <div style={{ textAlign: "left", minWidth: 0 }}>
            <div style={{ fontFamily: "'Noto Serif Thai',serif", fontWeight: 700, fontSize: 20, color: "var(--ink)", lineHeight: 1.15, whiteSpace: "nowrap" }}>{settings.siteTitle}</div>
            <div style={{ fontSize: 12.5, color: "var(--accent)", fontWeight: 600, whiteSpace: "nowrap" }}>{settings.siteSubtitle}</div>
          </div>
        </button>

        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          {searchOpen ? (
            <form onSubmit={doSearch} style={{ position: "relative", width: 200 }}>
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 20px" }}>
        {/* STATS */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 14, marginBottom: 52, marginTop: -70, position: "relative", zIndex: 2 }}>
          {stats.map((s) => (
...
function DepartmentDetail({ data, id, goto }) {
  ...
  return (
    <PageShell title={dept.name} subtitle={dept.nameEn} icon={GraduationCap} accent={deptAccent(data, dept.id)}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 28, marginBottom: 40 }}>
        <div>
...
function TeacherProfile({ data, id, goto }) {
  ...
  return (
    <PageShell title="">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 30, marginBottom: 36 }}>
        <div>
      ...
      <div className="sfg-card" style={{ padding: 22, display: "grid", gridTemplateColumns: "1fr auto", gap: 20, alignItems: "center", marginBottom: 40 }}>
...
function ClassroomDetail({ data, id, goto }) {
  ...
  return (
    <PageShell title={`ห้อง ${room.number}`} subtitle={`${bld?.name} · ชั้น ${room.floor} · ${room.type}`} icon={DoorOpen} accent={room.departmentId ? deptAccent(data, room.departmentId) : "#7D4FA6"}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 28, marginBottom: 36 }}>
...
/* =========================================================================
   ADMIN — DASHBOARD
   ========================================================================= */
function AdminDashboard({ data, persist, onLogout, goto }) {
  ...
  return (
    <div style={{ maxWidth: 1180, margin: "0 auto", padding: "30px 20px 64px", display: "grid", gridTemplateColumns: "220px 1fr", gap: 26 }}>
      <div>
...
export default function App() {
  const { data, status, saving, persist } = useFacultyData();
  ...
  switch (page.name) {
    ...
    case "admin-dashboard":
      content = isAdmin
        ? <AdminDashboard data={data} persist={persist} onLogout={() => setIsAdmin(false)} goto={goto} />
  ...
  return (
    <div className="sfg-app" data-theme={theme} style={{ minHeight: "100vh" }}>
      ...
      <footer style={{ borderTop: "1px solid var(--border)", background: "var(--surface-2)", marginTop: 40 }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 20px 24px", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr 1fr", gap: 28 }}>
          <div>
