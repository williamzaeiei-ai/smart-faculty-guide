  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 20px" }}>
        ...
        <SectionTitle eyebrow="แกลเลอรี" title="บรรยากาศภายในคณะ" action={<button onClick={() => goto("places")} className="sfg-btn sfg-btn-ghost">ดูสถานที่ทั้งหมด <ChevronRight size={15} /></button>} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(140px,1fr))", gap: 10 }}>
          {[...data.buildings, ...data.places].slice(0, 12).map((item, i) => {
            const isPlace = "openTime" in item;
            const meta = isPlace ? (PLACE_TYPES[item.type] || PLACE_TYPES.other) : null;
            return (
              <button
                key={item.id + i}
                onClick={() => goto(isPlace ? "place-detail" : "building-detail", item.id)}
                className="sfg-card sfg-card-hover"
                style={{ border: "1px solid var(--border)", padding: 0 }}
                title={item.name}
              >
                <Thumb icon={isPlace ? meta.icon : Building2} src={item.image} accent={isPlace ? meta.color : idxAccent(data, "buildings", item.id)} aspect="1/1" size={20} />
              </button>
            );
          })}
        </div>
