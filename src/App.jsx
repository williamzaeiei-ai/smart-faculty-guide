...
const GlobalStyle = () => (
  <style>{`
    ...
    .sfg-gallery-tile:hover .sfg-gallery-caption { opacity: 1; transform: translateY(0); }
...
/* Curated icon set for the editable "why use this system" home page feature cards.
   Stored as string keys in data (JSON-safe) and resolved to components here. */
const FEATURE_ICON_MAP = {
  search: Search, bot: Bot, navigation: Navigation, dashboard: LayoutDashboard,
  graduation: GraduationCap, building: Building2, mapPin: MapPin, user: User,
  door: DoorOpen, calendar: Calendar, clock: Clock, users: Users, info: Info, book: BookOpen,
};
const FEATURE_ICON_OPTIONS = [
  { value: "search", label: "ค้นหา (แว่นขยาย)" },
  { value: "bot", label: "AI / บอท" },
  { value: "navigation", label: "แผนผัง / นำทาง" },
  { value: "dashboard", label: "แดชบอร์ด" },
  { value: "graduation", label: "หมวกรับปริญญา" },
  { value: "building", label: "อาคาร" },
  { value: "mapPin", label: "หมุดแผนที่" },
  { value: "user", label: "บุคคล" },
  { value: "door", label: "ประตู/ห้อง" },
...
const DEFAULT_FEATURES = [
  { icon: "search", title: "ค้นหาได้ในที่เดียว", desc: "รวมข้อมูลอาจารย์ ห้องเรียน อาคาร และสถานที่สำคัญไว้ในระบบเดียว ไม่ต้องถามหลายที่" },
...
function seedData() {
  ...
  // Standalone "recommended photos" — plain images with a caption, not tied to any building/place/room.
  const gallery = [
    { id: "g1", image: "", caption: "บรรยากาศการเรียนการสอน" },
    { id: "g2", image: "", caption: "กิจกรรมนักศึกษา" },
    { id: "g3", image: "", caption: "พื้นที่ทำกิจกรรมภายในคณะ" },
    { id: "g4", image: "", caption: "ห้องปฏิบัติการทันสมัย" },
    { id: "g5", image: "", caption: "งานปัจฉิมนิเทศ" },
    { id: "g6", image: "", caption: "มุมพักผ่อนของนักศึกษา" },
  ];

  return {
    departments, buildings, classrooms, teachers, schedules, places, gallery,
    settings: {
      logoImage: "",
...
/* =========================================================================
   NAVBAR
   ========================================================================= */
function Navbar({ page, goto, isAdmin, onLogout, theme, toggleTheme, settings, query, setQuery, onSearchSubmit, overlay }) {
  ...
  // "overlay" = transparent, floating over the home hero (Motion Cover); solidifies once scrolled.
  const ghostBtnStyle = overlay
    ? { background: "rgba(255,255,255,0.14)", color: "#fff", border: "1px solid rgba(255,255,255,0.28)" }
    : {};
  const titleColor = overlay ? "#fff" : "var(--ink)";
  const subtitleColor = overlay ? "rgba(255,255,255,0.85)" : "var(--accent)";

  return (
    <div style={{
      position: overlay ? "fixed" : "sticky", top: 0, left: 0, right: 0, zIndex: 40, width: "100%",
          ...
          {searchOpen ? (
            <form onSubmit={doSearch} className="sfg-navbar-search" style={{ position: "relative", width: 200 }}>
              ...
              <input
                ...
                placeholder="ค้นหา..."
                className="sfg-input"
                style={{ paddingLeft: 30, paddingRight: 30, fontSize: 13, boxShadow: overlay ? "0 8px 20px -8px rgba(0,0,0,0.4)" : "none" }}
              />
            ...
            <button onClick={() => setSearchOpen(true)} className="sfg-btn sfg-btn-ghost" style={{ padding: 10, ...ghostBtnStyle }} title="ค้นหา">
              <Search size={17} />
            </button>
          )}

          <button onClick={toggleTheme} className="sfg-btn sfg-btn-ghost" style={{ padding: 10, ...ghostBtnStyle }} title="สลับโหมดมืด/สว่าง">
            {theme === "dark" ? <Sun size={17} /> : <Moon size={17} />}
          </button>

          {isAdmin ? (
            <button onClick={() => goto("admin-dashboard")} className="sfg-btn sfg-btn-primary" style={overlay ? { background: "#fff", color: "var(--primary)" } : {}}>
              <LayoutDashboard size={16} /> <span className="sfg-admin-btn-label">Dashboard</span>
            </button>
          ) : (
            <button onClick={() => goto("admin-login")} className="sfg-btn sfg-btn-ghost" style={ghostBtnStyle}>
              <LogIn size={16} /> <span className="sfg-admin-btn-label">Admin</span>
            </button>
          )}

          <button onClick={() => setMenuOpen((v) => !v)} className="sfg-btn sfg-btn-ghost" style={{ padding: 10, ...ghostBtnStyle }} title="เมนู">
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 20px" }}>
        {/* STATS */}
        <div className="sfg-hero-stats" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 14, marginBottom: 52, position: "relative", zIndex: 2 }}>
          {stats.map((s) => (
            <div key={s.label} className="sfg-card sfg-card-hover" style={{ border: "none" }}>
              <Thumb icon={s.icon} accent={s.color} aspect="1/1" size={26} />
              <div style={{ padding: "14px 16px" }}>
                <div style={{ fontFamily: "'Noto Serif Thai',serif", fontSize: 24, fontWeight: 700, color: "var(--ink)", lineHeight: 1 }}>{s.value}</div>
...
function DepartmentCard({ dept, data, onClick }) {
  ...
  return (
    <button onClick={onClick} className="sfg-card sfg-card-hover" style={{ textAlign: "left", border: "none" }}>
      <AccentBar color={accent} />
      <Thumb icon={GraduationCap} src={dept.image} accent={accent} aspect="1/1" />
...
function TeacherCard({ teacher, data, onClick }) {
  ...
  return (
    <button onClick={onClick} className="sfg-card sfg-card-hover" style={{ textAlign: "left", border: "none", position: "relative" }}>
      <AccentBar color={accent} />
      <div style={{ position: "relative" }}>
        <Thumb icon={User} src={teacher.image} accent={accent} aspect="1/1" />
        <span style={{
...
function ClassroomCard({ room, data, onClick }) {
  ...
  return (
    <button onClick={onClick} className="sfg-card sfg-card-hover" style={{ textAlign: "left", border: "none" }}>
      <AccentBar color={accent} />
      <Thumb icon={DoorOpen} label={room.type} src={room.image} accent={accent} aspect="1/1" />
      <div style={{ padding: 16 }}>
...
function BuildingCard({ b, data, onClick }) {
  ...
  return (
    <button onClick={onClick} className="sfg-card sfg-card-hover" style={{ textAlign: "left", border: "none" }}>
      <AccentBar color={accent} />
      <Thumb icon={Building2} src={b.image} accent={accent} aspect="1/1" />
      <div style={{ padding: 16 }}>
...
function PlaceCard({ place, data, onClick }) {
  ...
  return (
    <button onClick={onClick} className="sfg-card sfg-card-hover" style={{ textAlign: "left", border: "none" }}>
      ...
      <Thumb icon={meta.icon} label={meta.label} src={place.image} accent={meta.color} aspect="1/1" />
      <div style={{ padding: 16 }}>
      ...
      <div>
        ...
        {tab === "gallery" && (
          <GenericAdminTable
            ...
            onDelete={deleteItem("gallery")}
            fields={[
              { key: "caption", label: "คำบรรยายภาพ", showInList: true, placeholder: "เช่น บรรยากาศการเรียนการสอน" },
              { key: "image", label: "รูปภาพ (แนะนำสัดส่วน 1:1)", type: "image", wide: true },
            ]}
          />
        )}
...
function SettingsForm({ settings, onSave }) {
  ...
  return (
    <form onSubmit={save} className="sfg-card" style={{ padding: 22, maxWidth: 560, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ borderTop: "1px solid var(--border)", marginTop: 8, paddingTop: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 4 }}>
          <div>
            <label style={{ fontSize: 12, color: "var(--ink-soft)", display: "block", marginBottom: 4 }}>ข้อความปุ่ม CTA</label>
            <input className="sfg-input" value={form.heroCtaText || ""} onChange={set("heroCtaText")} placeholder="เช่น ดูข้อมูลเพิ่มเติม" />
          </div>
          <div>
            <label style={{ fontSize: 12, color: "var(--ink-soft)", display: "block", marginBottom: 4 }}>ปุ่มลิงก์ไปหน้า</label>
            <select className="sfg-input" value={form.heroCtaTarget || "departments"} onChange={set("heroCtaTarget")}>
              <option value="departments">สาขา</option>
              <option value="teachers">อาจารย์</option>
              <option value="classrooms">ห้องเรียน</option>
              <option value="buildings">อาคาร</option>
              <option value="places">สถานที่</option>
              <option value="map">แผนผัง</option>
              <option value="contact">ติดต่อ</option>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {(form.features || DEFAULT_FEATURES).map((f, i) => (
            <div key={i} style={{ border: "1px solid var(--border)", borderRadius: 10, padding: 12, display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ fontSize: 11.5, color: "var(--ink-soft)", fontWeight: 700 }}>การ์ดที่ {i + 1}</div>
              <select
                className="sfg-input"
                value={f.icon}
                onChange={(e) => {
                  const next = [...(form.features || DEFAULT_FEATURES)];
...
export default function App() {
  ...
  let content;
  switch (page.name) {
    case "home": content = <HomePage data={data} goto={goto} query={query} setQuery={setQuery} onSearchSubmit={onSearchSubmit} navOverlay={navOverlay} />; break;
  ...
  return (
    <div className="sfg-app" data-theme={theme} style={{ minHeight: "100vh" }}>
      ...
      <footer style={{ background: "#14151F", color: "#C7C9D6", marginTop: 40 }}>
        <div className="sfg-footer-grid" style={{ maxWidth: 1180, margin: "0 auto", padding: "44px 20px 28px" }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {[Info, Mail, Phone, MapPin].map((Icon, i) => (
                <span key={i} style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(255,255,255,0.08)", display: "flex", alignItems: "center", justifyContent: "center", color: "#C7C9D6" }}>
                  <Icon size={14} />
                </span>
              ))}
            </div>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 12, color: "#fff" }}>เมนูลัด</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {[["departments", "สาขา"], ["teachers", "อาจารย์"], ["classrooms", "ห้องเรียน"], ["buildings", "อาคาร"]].map(([k, l]) => (
                <button key={k} onClick={() => goto(k)} style={{ background: "none", border: "none", textAlign: "left", padding: 0, color: "#9497A8", fontSize: 12.5, cursor: "pointer" }}>{l}</button>
              ))}
            </div>
          </div>
          <div>
            <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 12, color: "#fff" }}>บริการ</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10, maxWidth: 1180, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 16, fontSize: 11.5, color: "#7B7E90" }}>
            <button onClick={() => goto("contact")} style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", padding: 0 }}>ติดต่อเรา</button>
            <button onClick={() => goto("map")} style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", padding: 0 }}>แผนผัง</button>
          </div>
        </div>
      </footer>
    </div>
